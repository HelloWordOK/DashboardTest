...
<% 
	...
	for(int i=12;i>=0;i--){
		...
		String week = "W"+DateUtils.getWeek(curDay, DateUtils.FIRST_DAY_EN);
	...
	String weekMapJson = JsonUtils.object2jackson(weekMap);
	String monthListJson = JsonUtils.object2jackson(monthList);
%>
...
<script type="text/javascript">
	...
	var path = '<%=basePath%>';
	// Get serviceCode period type
	var param = $("#serviceCode").val();
	...
	if(dataSource.chart1!=null){
			...
			toolbox : {
				...
				feature : {
					...
					myDownload :{
		                ...
		                onclick: function (){
		                	location.href = path+"qa/qa_data_export?param=" + param + "&periodType=" + period_type + "&chartType=incident&isDomain=false";
		                }
					}
				}
			},
		...
		chart1.on('click', function (params) {
			var series_name = params.seriesName;
			if(series_name=='Closed'||series_name=='% SLA'){
				var startTime = params.name;
				var endTime;
				if(startTime.startWith("W")){
					var weekMap = <%=weekMapJson%>;
					for(var week in weekMap){
						if(startTime==week){
							startTime = weekMap[week][0];
							endTime = weekMap[week][6];
							break;
						}
					}
				}else{
					var monthList = <%=monthListJson%>;
					for(var i in monthList){
						if(startTime==monthList[i]){
							startTime = monthList[i]+"-01";
							var end = monthList[parseInt(i)+1]+"-01";
							var d=new Date(end.replace(/-/g,"/"));
				...
				/* $.each(weekMap, function(i) {  
				    alert(i+"----"+weekMap[i]);         
				}); */
				var cookietime = 'h30';
				setCookie('serviceCode', $("#serviceCode").val(),cookietime);
				setCookie('dataType', "incident",cookietime);
				/* setCookie('dataStatus', "closed",cookietime); */
				setCookie('startMonth', startTime,cookietime);
				setCookie('endMonth', endTime,cookietime);
				if(series_name=='Closed'){
					setCookie('slaBreach', ['true','false'],cookietime);
				}else{
					setCookie('slaBreach', ['true'],cookietime);
				}
				window.open(path+'qa/service/data');
			}
			return false;
		});
	}
	...
	if(dataSource.chart3!=null){
			...
			toolbox : {
				...
				feature : {
					...
					myDownload :{
		                ...
		                icon: 'path://M4.7,22.9L29.3,45.5L54.7,23.4M4.6,43.6L4.6,58L53.8,58L53.8,43.6M29.2,45.1L29.2,0',
		                onclick: function (){
		                	location.href = path+"qa/qa_data_export?param="+param+"&periodType="+period_type + "&chartType=SR&isDomain=false";
		                }
					}
				}
			},
		...
		chart3.on('click', function (params) {
			...
			if(series_name=='Closed'||series_name=='% SLA'){
				...
				var cookietime = 'h30';
				setCookie('serviceCode', $("#serviceCode").val(),cookietime);
				setCookie('dataType', "sr",cookietime);
				/* setCookie('dataStatus', "closed",cookietime); */
				setCookie('startMonth', startTime,cookietime);
				setCookie('endMonth', endTime,cookietime);
				if(series_name=='Closed'){
...
/* 					saveAsImage : {
					...
					myDownload :{
		                ...
		                icon: 'path://M4.7,22.9L29.3,45.5L54.7,23.4M4.6,43.6L4.6,58L53.8,58L53.8,43.6M29.2,45.1L29.2,0',
		                onclick: function (){
		                	location.href = path+"qa/qa_data_export?param="+param+"&periodType="+period_type + "&chartType=problem&isDomain=false";
		                }
					}
				}
			},
		...
		chart5.on('click', function (params) {
			...
			if(series_name=='Closed'||series_name=='% SLA'){
				...
				var cookietime = 'h30';
				setCookie('serviceCode', $("#serviceCode").val(),cookietime);
				setCookie('dataType', "problem",cookietime);
				/* setCookie('dataStatus', "closed",cookietime); */
				setCookie('startMonth', startTime,cookietime);
				setCookie('endMonth', endTime,cookietime);
				if(series_name=='Closed'){
